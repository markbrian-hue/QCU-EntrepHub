...
const latestAnnouncement = ref(null); // For the Top Banner
const hasNew = ref(false);
const showBanner = ref(true); // Allow closing the banner
...
const fetchAnnouncements = async () => {
  try {
    ...
    // Set the latest one for the banner
    if (data.length > 0) {
...
onMounted(() => {
  checkUserStatus();
  fetchAnnouncements(); // Fetch immediately for everyone
});
...
<template>
  <div class="min-h-screen bg-white font-sans text-gray-900 flex flex-col">
    ...
    <div v-if="latestAnnouncement && showBanner" class="bg-gray-900 text-yellow-400 px-4 py-2 text-xs font-bold uppercase tracking-widest text-center relative z-[60]">
      <div class="container mx-auto flex justify-center items-center gap-3">
        ...
        <span>{{ latestAnnouncement.title }}: <span class="text-white font-medium normal-case tracking-normal">{{ latestAnnouncement.message }}</span></span>
        <button @click="showBanner = false" class="absolute right-4 text-gray-500 hover:text-white">✕</button>
      </div>
    </div>
    ...
    <nav class="bg-white border-b-4 border-yellow-500 sticky top-0 z-50 shadow-md">
      <div class="container mx-auto px-6 py-4 flex justify-between items-center">
        ...
        <div class="flex items-center gap-8 text-xs font-bold uppercase tracking-widest text-gray-500">
          ...
          <router-link v-if="!isVendor && !isAdmin" to="/cart" active-class="text-black" class="hover:text-yellow-600 transition-colors py-2 flex items-center gap-2 group">
            Cart
            <span v-if="cartStore.totalItems > 0" class="bg-black text-yellow-500 text-[10px] px-1.5 py-0.5 font-black group-hover:bg-yellow-500 group-hover:text-black transition-colors">
          ...
          <div v-if="isLoggedIn" class="relative">
            ...
            <div v-if="showNotifications" class="absolute right-0 mt-3 w-80 bg-white border border-gray-200 shadow-2xl z-50 animate-fade-in-up rounded-sm">
              <div class="p-4 border-b border-gray-100 bg-gray-50">
                <h4 class="text-xs font-black text-gray-900 uppercase tracking-wide">Announcements</h4>
              </div>
          ...
          <div v-else class="pl-6 border-l-2 border-gray-100">
            <router-link to="/login" class="px-6 py-3 bg-black text-white font-black hover:bg-yellow-500 hover:text-black transition-all duration-300 shadow-sm uppercase text-xs tracking-widest">Login</router-link>
          </div>
