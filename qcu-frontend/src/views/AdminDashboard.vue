...
const user = ref(null);
const allUsers = ref([]);
const announcements = ref([]); // New Data
...
const fetchData = async () => {
  try { const res = await api.get('/admin/users'); allUsers.value = res.data; } catch (err) { console.error(err); }
};

const fetchAnnouncements = async () => {
  try { const res = await api.get('/announcements'); announcements.value = res.data; } catch (err) { console.error(err); }
};
...
const triggerVerify = (vendorId, status) => {
  ...
  confirmConfig.value = {
    ...
    confirmBtnColor: isApprove ? 'bg-green-600' : 'bg-red-600',
    action: async () => {
      try { await api.put(`/admin/vendors/${vendorId}/verify`, { status }); toast.success(`Vendor ${status}`); await fetchData(); } 
...
const triggerDelete = (userId) => {
  confirmConfig.value = {
    ...
    action: async () => {
      try { await api.delete(`/admin/users/${userId}`); toast.success("User deleted."); await fetchData(); } 
      catch (err) { toast.error("Action failed"); }
    }
  };
  showConfirmModal.value = true;
...
<template>
  <div class="max-w-7xl mx-auto p-6 font-sans">
    ...
    <div v-if="activeTab === 'vendors'">
      <div class="bg-white border border-gray-200 shadow-sm overflow-x-auto">
        <table class="w-full text-left border-collapse">
          <thead><tr class="bg-gray-50 border-b border-gray-200 text-[10px] font-black text-gray-500 uppercase tracking-widest"><th class="p-4">Shop</th><th class="p-4">Owner</th><th class="p-4">ID</th><th class="p-4 text-right">Action</th></tr></thead>
    ...
    <div v-if="activeTab === 'buyers'">
      <div class="bg-white border border-gray-200 shadow-sm overflow-x-auto">
        <table class="w-full text-left border-collapse">
          <thead><tr class="bg-gray-50 border-b border-gray-200 text-[10px] font-black text-gray-500 uppercase tracking-widest"><th class="p-4">Name</th><th class="p-4">ID</th><th class="p-4">Photo</th><th class="p-4 text-right">Action</th></tr></thead>
