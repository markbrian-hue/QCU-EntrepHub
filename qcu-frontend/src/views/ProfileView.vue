...
// --- CRITICAL FIX: STOCK DEDUCTION LOGIC ---
const handleConfirmAction = async () => {
  ...
  try {
    if (action === 'CANCEL') {
      ...
      // 1. Fetch Items for this order to know what to subtract
      ...
      <div class="md:col-span-1">
        <div class="bg-gray-900 text-white p-8 border-l-8 border-yellow-500 sticky top-24 shadow-xl">
          ...
          <div class="space-y-6 text-sm">
            <div>
              <span class="block text-[10px] text-gray-500 uppercase font-bold tracking-widest mb-1">Email / ID</span>
      ...
      <div class="md:col-span-2">
        ...
        <div v-else class="space-y-6">
          <div 
            ...
            class="bg-white border border-gray-200 p-0 transition group shadow-sm relative hover:border-black hover:shadow-md"
            :class="(order.status === 'COMPLETED' || order.status === 'CANCELLED') ? 'opacity-70 bg-gray-50' : 'cursor-pointer'"
          >
            ...
            <div class="p-6">
              ...
              <div class="flex justify-between items-center border-t border-gray-100 pt-4" @click.stop>
                <button v-if="order.status === 'PENDING'" @click="triggerCancel(order.orderId)" class="text-xs font-bold text-red-500 hover:text-red-700 hover:underline uppercase tracking-wide">
                  Cancel Order
                </button>
                <span v-else-if="order.status === 'CANCELLED'" class="text-xs font-bold text-gray-400 uppercase italic">Order Cancelled</span>
                <span v-else class="text-xs font-bold text-gray-400 uppercase italic">Processing Started</span>
    ...
    <div v-if="showDetailsModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="bg-white w-full max-w-lg relative z-10 shadow-2xl border-t-8 border-yellow-500 animate-fade-in-up p-8">
        ...
        <div v-else class="space-y-4 max-h-60 overflow-y-auto pr-2">
            <div v-for="item in orderItems" :key="item.name" class="flex gap-4 items-center bg-gray-50 p-3 border border-gray-200">
                <img :src="item.imageUrl || 'https://via.placeholder.com/50'" class="w-12 h-12 object-cover border border-gray-300" />
                <div class="flex-1">
        ...
        <div v-if="selectedOrder?.status === 'READY' || selectedOrder?.status === 'READY_FOR_PICKUP'" class="mt-8">
            <button @click="triggerReceive(selectedOrder.orderId)" class="w-full bg-black text-white py-4 font-bold uppercase tracking-widest hover:bg-green-600 transition shadow-lg">
                Confirm Receipt
            </button>
        </div>
      </div>
    ...
    <div v-if="showConfirmModal" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div class="bg-white w-full max-w-sm relative z-10 p-6 shadow-2xl border-l-8 animate-fade-in-up" 
          ...
          <div class="flex gap-3">
            <button @click="showConfirmModal = false" class="flex-1 py-3 bg-gray-100 text-gray-600 font-bold uppercase text-xs hover:bg-gray-200">No, Go Back</button>
            <button @click="handleConfirmAction" class="flex-1 py-3 text-white font-bold uppercase text-xs shadow-md" :class="confirmConfig.confirmButtonColor">{{ confirmConfig.confirmButtonText }}</button>
          </div>
      </div>
    </div>
  </div>
